...
(Claude Desktop, Cline, Cursor, agent frameworks) can call its verification
tools directly:

- `verify_receipt` — verify a single Decision Receipt against an Ed25519 key.
- `verify_chain` — verify an ordered JSONL hash chain.
- `verify_evidence_package` — verify a base64-encoded evidence ZIP offline.
- `compliance_summary` — return aggregate metrics (reject_rate, allow_rate,
  SPRT status, action_distribution, …).
- `list_compliance_controls` — read-only crosswalk to ISO 42001, NIST AI RMF,
  EU AI Act controls; optionally enriched with coverage / status when
  receipts are passed in.

The server **only exposes read-only verification**. Signing, key generation,
and `ReceiptCollector.emit` remain in the application's trust boundary —
agents cannot forge audit entries.

Install: `pip install "ai-audit-trail[mcp]"`. Run: `ai-audit-mcp` (new
console-script). Configure in Claude Desktop / Cline / Cursor as a stdio
MCP server pointing at `ai-audit-mcp`.

### Added — `llms.txt` + `llms-full.txt` for the docs site
Per the [llms.txt proposal](https://llmstxt.org/), the documentation site now
serves a deterministic LLM-readable manifest (`llms.txt`) plus a concatenated
full-docs payload (`llms-full.txt`, ~19 KiB). AI tools that consume docs —
Cursor's `@docs`, Context7, Anthropic Skills — can ingest the package's
documentation without scraping HTML. Generated by `scripts/build_llms_full.py`
in the GitHub Pages workflow from the same MkDocs sources, so there is no
sync drift.

Live at:
- https://sundsoffice-tech.github.io/ai-audit-trail/llms.txt
- https://sundsoffice-tech.github.io/ai-audit-trail/llms-full.txt

### Quality
253/253 tests (+19 new), ruff clean, mypy strict 0 errors across 38 modules.
